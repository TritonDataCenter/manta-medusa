...
function
mdr_get_ip_address(log)
{
	var retaddr = null;
	var ifaces = mod_os.networkInterfaces();
...
MedusaReflector(logger, config_filename)
{
	...
	this.mdr_log.debug(this.mdr_conf, 'configuration');
	...
	this.mdr_identity = {
		...
		medusa_ip: mdr_get_ip_address(this.mdr_log),
	...
	var init_asset = function(_, next) {
		...
		mod_asset.put_medusa_agent(maopts, function(err) {
			if (err) {
				...
				log.debug('manta agent asset uploaded');
			}
			next(err);
		});
	};
