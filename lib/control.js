...
_send_wait_pulse()
{
	 ...
	 */
	if (!this.mds_shed_master) {
		self._cancel_wait_pulse();
		return;
	}
	this.mds_shed_master.send(mds_generate_message({ type: 'wait' }));
}
...
_init_moray_buckets(_cb)
{
	...
	moray.putBucket(MD_BUCKET_SESSIONS, opts, function (err) {
		mod_assert.ifError(err, 'moray: putBucket: failed');
		_cb();
	});
}
...
_init_moray(_cb)
{
	...
	moray = this.mdc_moray = mod_moray.createClient(config);
	...
	moray.on('close', function () {
		log.error(err, 'moray client closed');
	});

	moray.on('connect', function () {
		log.info('moray connected');
		self._init_moray_buckets(function () {
